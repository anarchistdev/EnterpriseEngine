...
public class ParticleSystem {
    ...
    private void emitParticle(Vector3f center) {
        ...
        ParticleManager.addParticle(new Particle(new Vector3f(center), velocity, gravityComplient, lifeLength, generateRotation(), scale, texture));
    }

    private float generateValue(float average, float errorMargin) {
        float offset = (MathUtil.randomNextFloat() - 0.5f) * 2f * errorMargin;
        return average + offset;
    }

    private float generateRotation() {
        if (randomRotation) {
            return MathUtil.randomNextFloat() * 360f;
        } else {
            return 0;
        }
    }
    ...
    private static Vector3f generateRandomUnitVectorWithinCone(Vector3f coneDirection, float angle) {
        ...
        float theta = (float) (MathUtil.randomNextFloat() * 2f * Math.PI);
        float z = cosAngle + (MathUtil.randomNextFloat() * (1 - cosAngle));
    ...
    private Vector3f generateRandomUnitVector() {
        float theta = (float) (MathUtil.randomNextFloat() * 2f * Math.PI);
        float z = (MathUtil.randomNextFloat() * 2) - 1;
        float rootOneMinusZSquared = (float) Math.sqrt(1 - z * z);
        float x = (float) (rootOneMinusZSquared * Math.cos(theta));
        float y = (float) (rootOneMinusZSquared * Math.sin(theta));
